...
%   acquisition
final_trial_length = 25; % The length of each data trial after processing,
%   taken from the tail of each full data trial.
%    That is, we discard some data from the start of each trial.
minimum_length_trial=23; % Any trial shorter than this will be discarded

spike_remover_params(1)=0; % The window size to use for spike removal
spike_remover_params(2)=0; % The max acceptable proportion change in the window
spike_remover_params(3)=0; % The max acceptable change in SD units in the window
sgolay_span = 20;         % Parameters of the SGolay interpolation prior
sgolay_polynomial = 7;    %   to interpolation
BadPercentChangeThreshold = 10.0;       % Any time point with an absolute change from
...
Protocols={'SPP_protocolA_Peri_Dilate_5mmPinhole','SPP_protocolB_Peri_Dilate_5mmPinhole','SPP_protocolC_Peri_Dilate_5mmPinhole'};
configFileNames=[];
% configFileNames = {'SPP-SIsolatingStrictPhotonIncrease-OLEyeTrackerLongCable.cfg' ; ...
%     'SPP-SIsolatingStrictPhotonDecrease-OLEyeTrackerLongCable.cfg' ; ...
%     'SPP-LMDirectedStrictPhotonIncrease-OLEyeTrackerLongCable.cfg' ; ...
%     'SPP-LMDirectedStrictPhotonDecrease-OLEyeTrackerLongCable.cfg'};
...
StackPlotFlag=0; % When set, multiple-subject results will be overlayed in
% a single plot
...
newLabels = {'LM-Decrease' ; 'LM-Increase' ; 'S-Decrease' ; 'S-Increase'};
oldLabels = {'SPP-LMDirectedStrictPhotonDecrease';...
    'SPP-LMDirectedStrictPhotonIncrease';...
    'SPP-SIsolatingStrictPhotonDecrease';...
    'SPP-SIsolatingStrictPhotonIncrease' };


ResultsDirName = 'SPP_25secWindow';
